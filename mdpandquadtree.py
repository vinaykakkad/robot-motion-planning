...
class PathPlanning:
    ...
    def __init__(self, maxRow=50, maxCol=50, num_obstacle_pts=50, cor_pr=0.7, wr_pr=0.1, n_actions=4, startRow=5,
                 startCol=5, goalRow=45, goalCol=45, goalReward=100, obstReward=-500, stayReward=-5, gamma=0.9):
        ...
        self.st = np.zeros((self.n_actions, self.maxRow *
                            self.maxCol, self.maxRow * self.maxCol))
    ...
    def get_obstacles(self):
        ...
        if (self.startCol < self.goalCol):
            ...
            SameCol = 1
            ColRange = list(range(0, self.goalCol)) + list(range(self.goalCol + 1, self.startCol)) + list(range(self.startCol + 1, self.maxCol - 1))

            if (SameRow == 1):
                raise ValueError("Start state and goal state are same")

        wallRows = [random.choice(RowRange)
                    for row in range(self.num_obstacle_pts)]
        wallCols = [random.choice(ColRange)
                    for col in range(self.num_obstacle_pts)]
    ...
    def build_st_trans_matrix(self):
        ...
        if ((self.cor_pr + (self.n_actions - 1) * self.wr_pr) < 0.99 or 
            (self.cor_pr + (self.n_actions - 1) * self.wr_pr) > 1.01 ):
        ...
        for row in range(self.maxRow):
            for col in range(self.maxCol):
                ...
                if (cur_occ == self.not_occupied):
                    ...
                    action_map = [
                                right_state, top_right_state, top_state, 
                                top_left_state, left_state, bottom_left_state,
                                bottom_state, bottom_right_state
                    ]
                    occ_map = [
                            right_occ, top_right_occ, top_occ, top_left_occ, 
                            left_occ, bottom_left_occ, bottom_occ,
                            bottom_right_occ,
                    ]
                    ...
                    for action in range(self.n_actions):
                        ...
                        for inner_action in range(self.n_actions):
                            ...
                            if (inner_action == 0):
                                ...
                                self.st[action][int(cur_state)][int(
                                    action_map_rot[inner_action])] = self.cor_pr
                                prob_sum += self.cor_pr

                            # **** DOUBT ****
                            else:
                                self.st[action][int(cur_state)][int(
                                    action_map_rot[inner_action])] = self.wr_pr
                                prob_sum += self.wr_pr
            ...
            for col in range(self.maxCol):
                ...
                if (cur_occ == self.not_occupied):
                    ...
                    if (bottom_right_occ != self.not_occupied):
                        self.rm[cur_state][bottom_right_action] = self.obstReward

                    if (bottom_right_state == goal_state):
                        self.rm[cur_state][bottom_right_action] = self.goalReward

        return None


def get_reward(startRow, startCol, goalRow, goalCol, oCol, oRow, num_states, m, optimal_policy, rm, maxRow, maxCol):
    cur_row = startRow
    cur_col = startCol
    ...
    while (1):
       ...
       # if(new_row > maxRow-1 or new_col > maxCol-1):
        #    print(cur_row, cur_col, new_row, new_col, cur_opt_action)
        if(new_row > maxRow-1 or new_col > maxCol-1):
            #new_row = maxRow-2
            #new_col = maxCol-2  
            break
...
def fit_policy(st, rm, gamma, num_states):
    ...
    policies = [dict(zip(list(range(num_states)), list(opt_policy)))
...
def plot_analysis(file_data_world, file_data_value, file_data_policy, obstReward, maxRow, maxCol):
    iterations = list(range(1, 1000, 10))
    ...
    for itr in iterations:
        ...
        value_time_value.append(
            sum(data_value[str(itr)]['time_iter']) / len(data_value[str(itr)]['time_iter']))
    ...
    visualize_policy(data_world['maxRow'], data_world['maxCol'], data_world['startRow'], data_world['startCol'],
                     ...
                     data_world['rm'], 'policy_iteration', obstReward)
    ...
    visualize_policy(data_world['maxRow'], data_world['maxCol'], data_world['startRow'], data_world['startCol'],
                     ...
                     data_world['rm'], 'value_iteration', obstReward)
    ...
    for p_pol in policy_policy:
        reward_p = get_reward(data_world['startRow'], data_world['startCol'], data_world['goalRow'],
                              ...
                              data_world['m'], p_pol, data_world['rm'], data_world['maxRow'], data_world['maxCol'])
    ...
    for v_pol in policy_value:
        reward_v = get_reward(data_world['startRow'], data_world['startCol'], data_world['goalRow'],
                              data_world['goalCol'], data_world['oCol'], data_world['oRow'], data_world['num_states'],
                              data_world['m'], v_pol, data_world['rm'], data_world['maxRow'], data_world['maxCol'])
...
def main(userInputData):
    ...
    data_policy = fit_policy(
        world_data['st'], world_data['rm'], world_data['gamma'], world_data['num_states'])
    ...
    # Showing plots
    plot_analysis('data_world.txt', 'data_value.txt', 'data_policy.txt', world_params['obstReward'], world_params['maxRow'], world_params['maxCol'])
